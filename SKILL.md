...
### 5. 最終流程稽核與完成度回報
- **完成前必須逐項檢查**：在回覆使用者「已完成」之前，必須依照本 skill 的工序逐一檢查實際產物與執行紀錄，不可只檢查是否有簡報檔。
- **建立完成度清單**：至少檢查並回報下列項目：
  1. 是否詢問並記錄簡報風格與影片段落範圍。
  2. 是否保留 `output/video.mp4`、`output/audio.mp3`、`output/subtitles.srt`、`output/transcript.txt`；若使用 YouTube 自動字幕或其他替代來源，必須明確註記原因。
  3. 是否詢問 NotebookLM 筆記架構；若未使用，必須說明是使用者未提供或本次未連接。
  4. 是否完成具時間碼的「描述－詮釋－反思」分析稿。
  5. 是否完成 15-20 頁的 PPTX 與 HTML 簡報。
  6. 是否逐頁建立「投影片文字／SRT 或逐字稿段落／時間碼／原始截圖／重繪圖」對照。
  7. 是否產生 `output/screenshots/slide_{N}.png` 原始影格，且數量與需要插圖的投影片頁面相符。
  8. 是否以原始截圖作為 ImagePaths / `referenced_image_paths` 基底完成 GPTimage2 重繪，並輸出至 `output/drawings/slide_{N}.png`。
  9. 是否確認簡報插圖實際引用 `output/drawings/`，並檢查 PPTX/HTML 可開啟。
- **未完成不得模糊帶過**：若任何項目未完成，最終回覆必須以「已完成 / 未完成 / 以替代方式完成」標示，並列出未完成原因、已產出的檔案、以及下一步補齊方式。
- **禁止過度宣稱**：只有當上述必要項目都完成或明確取得使用者同意跳過時，才能說「已完整依照 SLC skill 流程完成」。
...
* **未做最終流程稽核**：只回報「檔案已產生」而沒有逐項檢查 skill 工序，會讓未完成的字幕、逐頁截圖、ImagePaths 重繪或 NotebookLM 對齊被誤認為已完成。
